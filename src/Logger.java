...
public class Logger {
	...
	public void outputLog() {
		...
		if (FixParameter.ACTION_LOG) {
			int i = 0;
			File actionFile = new File("./log/Action/Action" +"_"+ FightingICE_AI_FDG.class.getName() + (player?0:1) + "_" + i + ".txt");
			while (actionFile.exists()) {
				actionFile = new File("./log/Action/Action" +"_"+ FightingICE_AI_FDG.class.getName() + (player?0:1) + "_" + i + ".txt");
				i++;
			}
			try {
		...
		if (FixParameter.PLAY_FEATURE_LOG) {
			int i = 0;
			File playarcFile = new File("./log/Playarc/Playarc" +"_"+ FightingICE_AI_FDG.class.getName() + (player?0:1)  + "_" + i + ".txt");
			while (playarcFile.exists()) {
				playarcFile = new File("./log/Playarc/Playarc" +"_"+ FightingICE_AI_FDG.class.getName() + (player?0:1)  + "_" + i + ".txt");
				i++;
			}
			try {
		...
		if (FixParameter.SEARCHNUM_LOG) {
			int i = 0;
			File playarcFile = new File("./log/searchNum/searchNum" +"_"+ FightingICE_AI_FDG.class.getName() + (player?0:1)  + "_" + i + ".txt");
			while (playarcFile.exists()) {
				playarcFile = new File("./log/searchNum/searchNum" +"_"+ FightingICE_AI_FDG.class.getName() + (player?0:1)  + "_" + i + ".txt");
				i++;
			}
			try {
				playarcFile.createNewFile();
